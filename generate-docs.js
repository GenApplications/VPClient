...
fs.readdir(inputDir, (err, files) => {
  ...
  files.forEach((file) => {
    ...
    codeBlocks.forEach((codeBlock) => {
      ...
      codeBlock.classList.add('rounded');
    });
...
function getSidebarItems(files, currentFile) {
  return files
    .map((file) => {
      ...
      return `<li><a href="${fileLink}" class="hover:text-blue-600  py-2 text-gray-600 transition-colors duration-300 transform rounded-lg hover:bg-gray-100 ${isActive}">${capitalizedFileName}</a></li>`;
    })
    .join('');
}
